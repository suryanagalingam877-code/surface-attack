...
export default function Dashboard({ scan, onReset }) {
  ...
  return <main className="content">
    ...
    <Section eyebrow="Controls" title="Security headers"><Table rows={headers} empty="No security header data returned." columns={[{ key: 'header', label: 'Header', render: (r) => r.header || r.name }, { key: 'status', label: 'Status', render: (r) => r.status || r.classification || (r.present ? 'Present' : 'Not observed') }, { key: 'observed_value', label: 'Observed value', render: (r) => r.observed_value || r.value || 'Not observed' }, { key: 'severity', label: 'Severity' }, { key: 'recommendation', label: 'Recommendation' }]} /></Section>
